            ...
            <form method="POST" action="{{ route('admin.employees.update', [$employee->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('position') ? 'is-invalid' : '' }}" type="text"
                        name="position" id="position" value="{{ old('position', $employee->position) }}" required>
                    @if ($errors->has(''))
                        <div class="invalid-feedback">
                            {{ $errors->first('') }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('badges') ? 'is-invalid' : '' }}" name="badges[]"
                        id="badges" multiple>
                        @foreach ($badges as $id => $badges)
                            <option value="{{ $id }}"
                                {{ in_array($id, old('badges', [])) || $employee->badges->contains($id) ? 'selected' : '' }}>
                                {{ $badges }}</option>
                        @endforeach
                    </select>
                    @if ($errors->has(''))
